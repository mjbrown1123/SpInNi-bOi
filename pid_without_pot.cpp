/*
	...
	TO DO:
	- Calibrate the gyro sensor using the calibration code
		- (offsets in the setup() function need to be changed)
...
// PROVIDE MORE DAMN DOCUMENTATION
...
void pid(float pitch, float roll) {
	...
	*/
	...
	speed1 = base_speed + d_theta_p;
	speed3 = base_speed - d_theta_p;

	// motor2 is part of axis when lowered, gyro values go positive
	speed2 = base_speed + d_theta_r;
	speed4 = base_speed - d_theta_r;

	s1.writeMicroseconds((int)speed1);
	s2.writeMicroseconds((int)speed2);
	s3.writeMicroseconds((int)speed3);
	s4.writeMicroseconds((int)speed4);
}
